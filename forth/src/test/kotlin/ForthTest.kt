...
abstract class ForthTest {
  ...
  class Addition : ForthTest() {
    ...
    @Test fun `can add two numbers`() = assertEquals(listOf(3), Forth().evaluate("1 2 +"))
  ...
  class Subtraction : ForthTest() {
    ...
    @Test fun `can subtract two numbers`() = assertEquals(listOf(-1), Forth().evaluate("3 4 -"))
  ...
  class Multiplication : ForthTest() {
    ...
    @Test fun `can multiply two numbers`() = assertEquals(listOf(8), Forth().evaluate("2 4 *"))
  ...
  class Division : ForthTest() {
    ...
    @Test fun `can divide two numbers`() = assertEquals(listOf(4), Forth().evaluate("12 3 /"))

    @Test fun `performs integer division`() = assertEquals(listOf(2), Forth().evaluate("8 3 /"))
  ...
  class CombinedArithmetic : ForthTest() {
    ...
    @Test fun `addition and subtraction`() = assertEquals(listOf(-1), Forth().evaluate("1 2 + 4 -"))
  ...
  class Drop : ForthTest() {
    ...
    fun `removes the top value on the stack if it is the only one`() =
        assertEquals(listOf(), Forth().evaluate("1 drop"))
  ...
  class UserDefinedWords : ForthTest() {
    ...
    @Test
    fun `can consist of built-in words`() =
        assertEquals(listOf(1, 1, 1), Forth().evaluate(": dup-twice dup dup ;", "1 dup-twice"))

    @Test
    fun `execute in the right order`() =
        assertEquals(listOf(1, 2, 3), Forth().evaluate(": countup 1 2 3 ;", "countup"))

    @Test
    fun `can override other user-defined words`() =
        assertEquals(listOf(1, 1, 1), Forth().evaluate(": foo dup ;", ": foo dup dup ;", "1 foo"))

    @Test
    fun `can override built-in words`() =
        assertEquals(listOf(1, 1), Forth().evaluate(": swap dup ;", "1 swap"))

    @Test
    fun `can override built-in operators`() =
        assertEquals(listOf(12), Forth().evaluate(": + * ;", "3 4 +"))

    @Test
    fun `can use different words with the same name`() =
        assertEquals(
            listOf(5, 6),
    ...
    @Test
    fun `can define word that uses word with the same name`() =
        assertEquals(listOf(11), Forth().evaluate(": foo 10 ;", ": foo foo 1 + ;", "foo"))
  ...
  class CaseInsensitivity : ForthTest() {
    ...
    @Test
    fun `DUP is case-insensitive`() =
        assertEquals(listOf(1, 1, 1, 1), Forth().evaluate("1 DUP Dup dup"))

    @Test
    fun `DROP is case-insensitive`() =
        assertEquals(listOf(1), Forth().evaluate("1 2 3 4 DROP Drop drop"))

    @Test
    fun `SWAP is case-insensitive`() =
        assertEquals(listOf(2, 3, 4, 1), Forth().evaluate("1 2 SWAP 3 Swap 4 swap"))

    @Test
    fun `OVER is case-insensitive`() =
        assertEquals(listOf(1, 2, 1, 2, 1), Forth().evaluate("1 2 OVER Over over"))

    @Test
    fun `user-defined words are case-insensitive`() =
        assertEquals(listOf(1, 1, 1, 1), Forth().evaluate(": foo dup ;", "1 FOO Foo foo"))

    @Test
    fun `definitions are case-insensitive`() =
        assertEquals(listOf(1, 1, 1, 1), Forth().evaluate(": SWAP DUP Dup dup ;", "1 swap"))
  }
}
