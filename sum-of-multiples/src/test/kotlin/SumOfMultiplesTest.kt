...
class SumOfMultiplesTest {
  @Test fun `no multiples within limit`() = assertEquals(0, SumOfMultiples.sum(setOf(3, 5), 1))
  ...
  @Test
  fun `more than one multiple within limit`() = assertEquals(9, SumOfMultiples.sum(setOf(3), 7))
  ...
  @Test fun `much larger limit`() = assertEquals(233168, SumOfMultiples.sum(setOf(3, 5), 1000))

  @Test fun `three factors`() = assertEquals(51, SumOfMultiples.sum(setOf(7, 13, 17), 20))

  @Test fun `factors not relatively prime`() = assertEquals(30, SumOfMultiples.sum(setOf(4, 6), 15))
  ...
  @Test
  fun `much larger factors`() = assertEquals(2203160, SumOfMultiples.sum(setOf(43, 47), 10000))

  @Test
  fun `all numbers are multiples of 1`() = assertEquals(4950, SumOfMultiples.sum(setOf(1), 100))

  @Test
  fun `no factors means an empty sum`() = assertEquals(0, SumOfMultiples.sum(emptySet(), 10000))

  @Test fun `the only multiple of 0 is 0`() = assertEquals(0, SumOfMultiples.sum(setOf(1), 1))
