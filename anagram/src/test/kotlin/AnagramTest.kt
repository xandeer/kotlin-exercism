...
class AnagramTest {
  ...
  @Test
  fun `no matches`() =
      anagramsOf("diaper").searchedIn("hello", "world", "zombies", "pants").shouldBeEmpty()

  @Test
  fun `detects two anagrams`() =
      anagramsOf("master").searchedIn("stream", "pigeon", "maters").shouldBeOnly("maters", "stream")

  @Test
  fun `does not detect anagram subsets`() =
      anagramsOf("good").searchedIn("dog", "goody").shouldBeEmpty()
  ...
  fun `detects multiple anagrams with different case`() =
      anagramsOf("nose").searchedIn("Eons", "ONES").shouldBeOnly("Eons", "ONES")

  @Test
  fun `does not detect non-anagrams with identical checksum`() =
      anagramsOf("mass").searchedIn("last").shouldBeEmpty()
  ...
  fun `does not detect an anagram if the original word is repeated`() =
      anagramsOf("go").searchedIn("go Go GO").shouldBeEmpty()

  @Test
  fun `anagrams must use all letters exactly once`() =
      anagramsOf("tapper").searchedIn("patter").shouldBeEmpty()

  @Test
  fun `words are not anagrams of themselves (case-insensitive)`() =
      anagramsOf("BANANA").searchedIn("Banana").shouldBeEmpty()

  @Test
  fun `words other than themselves can be anagrams`() =
      anagramsOf("LISTEN").searchedIn("Listen", "Silent", "LISTEN").shouldBeOnly("Silent")
}
...
private fun Set<String>.shouldBeOnly(vararg expectation: String) =
    assertEquals(setOf(*expectation), this)
